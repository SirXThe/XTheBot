...
async def counting_update_entry(guild_id: int, channel_id: int, mode: str, count: int,
    ...
    async with aiosqlite.connect("database/database.db") as db:
        ...
        await db.commit()
        logging.info(f"[Counting] Updated counting entry for {guild_id}, set it to {channel_id}, {mode}, {count}, "
                     f"{last_user}, {last_counted}, {resets}, {record}, {record_user}, {record_time}")
